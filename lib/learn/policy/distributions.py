...
from lib.nn.mlp import build_mlp
...
class Categorical(torch.distributions.Categorical):
    ...
    def imitation_accuracy(self, actions, mask=None, reduction="mean", scale_100=False):
        ...
        return classify_accuracy(self.logits, actions, mask=mask, reduction=reduction, scale_100=scale_100)
...
class MixtureOfGaussian:
    ...
    def mode(self):
        ...
        if self._low_noise_eval:
            ...
            component_distribution = torch.distributions.Normal(loc=self._means, scale=scales)
            ...
            one_hot = F.one_hot(self._logits.argmax(dim=-1), self._logits.shape[-1]).unsqueeze(-1)
...
class GMMHead(nn.Module):
    def __init__(
        ...
        *,
        ...
        if mean_mlp_last_layer_gain is not None:
            ...
            nn.init.orthogonal_(self._mean_mlp[-1].weight, gain=mean_mlp_last_layer_gain)
    ...
    def forward(self, x: torch.Tensor):
        ...
        return MixtureOfGaussian(logits, mean, scale, low_noise_eval=self._low_noise_eval)
