...
from lib.utils.tree_utils import stack_sequence_fields, unstack_sequence_fields
                    ...
                    if prev_done_count > 0:
                        ...
                        for prev_done_idx in prev_done_indices:
                            ...
                            if len(rollouts[prev_done_idx]) < self.min_episode_length:
                                ...
                                rollout_to_save = stack_sequence_fields(rollout_to_save)  # dict of (T + 1, ...)
                                rollout_to_save["actions"] = actions_to_save
                                # discard first step since the data are wrong due to IG's bug
                                rollout_to_save = {k: v[1:] for k, v in rollout_to_save.items()}
                                ...
                                elif to_save_flag and not self.save_successful_rollouts_only:
                                    if successful:
                                        rollout_grp = self.s_grp.create_group(
                                            f"rollout_{self.saved_successful_rollouts}"
                                        )
                                    else:
                                        rollout_grp = self.f_grp.create_group(f"rollout_{self.saved_failed_rollouts}")
                                    for k, v in rollout_to_save.items():
                                        rollout_grp.create_dataset(k, data=v)
                                    if successful:
                                        self.saved_successful_rollouts += 1
...
class MyPPOPlayerContinuous(MyBasePlayer):
    def __init__(self, *args, **kwargs):
        ...
        self.normalize_input_excluded_keys = self.config.get("normalize_input_excluded_keys", None)
    ...
    def get_action(self, obs, is_deterministic=False):
        ...
        if "base_actions" in res_dict:
            current_action = torch.cat([res_dict["base_actions"], current_action], dim=1)
        if self.has_batch_dimension == False:
            current_action = torch.squeeze(current_action.detach())

        if self.clip_actions:
            if self.use_soft_clamp:
                clamped_action = soft_clamp(
                    current_action, torch.ones_like(current_action) * -1.0, torch.ones_like(current_action)
                )
            else:
                clamped_action = torch.clamp(current_action, -1.0, 1.0)
    ...
    def restore(self, fn):
        if type(fn) is str:
            ...
            ckp_list = []
            for checkpoint in fn:
                checkpoint = load_checkpoint(checkpoint)
                ckp_list.append(checkpoint)
                assert "running_mean_std" not in checkpoint, "running_mean_std is not supported for multiple models"
                assert (
                    "env_state" not in checkpoint or checkpoint["env_state"] is None
                ), "env_state is not supported for multiple models"
            self.model.load_state_dict([ckp["model"] for ckp in ckp_list])
