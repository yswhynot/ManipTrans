...
def rescale_actions(low, high, action):
    ...
    if d.shape[0] * 2 == action.shape[1]:  # handle residual actions
        d = torch.cat([d, d], dim=0)
        m = torch.cat([m, m], dim=0)
    elif (d.shape[0]) * 2 + 3 * 2 == action.shape[1]:  # BiH, handle PID actions, since PID use 6Drot
        dof_d = d.shape[0] // 2 - 6
        d = torch.cat(
            [
                d[:6],
                ...
                d[6 : 6 + dof_d],
                d[6 + dof_d : 6 + dof_d + 6],
                d[6 + dof_d + 3 : 6 + dof_d + 6],
                d[6 + dof_d + 6 : 6 + dof_d + 6 + dof_d],
                d,
            ],
            dim=0,
        )
        m = torch.cat(
            [
                ...
                m[6 : 6 + dof_d],
                m[6 + dof_d : 6 + dof_d + 6],
                m[6 + dof_d + 3 : 6 + dof_d + 6],
                m[6 + dof_d + 6 : 6 + dof_d + 6 + dof_d],
                m,
            ],
            dim=0,
        )
    elif (d.shape[0]) * 2 + 3 == action.shape[1]:  # Single H, handle PID actions, since PID use 6Drot
        d = torch.cat([d[:6], d[3:6], d[6:], d], dim=0)
        m = torch.cat([m[:6], m[3:6], m[6:], m], dim=0)
...
class MyA2CBase(BaseAlgorithm):
    def __init__(self, base_name, params):
        ...
        if full_experiment_name:
            ...
            self.experiment_name = config["name"] + pbt_str + datetime.now().strftime("_%m-%d-%H-%M-%S")
        ...
        self.early_stop_epochs = self.config.get("early_stop_epochs", 500)
        self.max_frames = self.config.get("max_frames", -1)

        self.is_adaptive_lr = config["lr_schedule"] == "adaptive"
        self.linear_lr = config["lr_schedule"] == "linear"
        self.is_warmup_lr = config["lr_schedule"] == "warmup"
        ...
        self.bptt_len = self.config.get(
            "bptt_length", self.seq_length
        ...
        self.normalize_input_excluded_keys = self.config["normalize_input_excluded_keys"]
        ...
        self.obs = None
        self.games_num = (
            self.config["minibatch_size"] // self.seq_length
        ...
        self.minibatch_size = min(self.minibatch_size, self.num_actors * self.num_agents)
    ...
    def clear_stats(self):
        ...
        for k, v in self.game_reward_dict.items():
            v.clear()
    ...
    def set_param(self, param_name, param_value):
        if param_name in [
            ...
            if self.global_rank == 0:
                if self.is_adaptive_lr or self.is_warmup_lr:
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            if torch.any(self.dones):
                self.obs, _ = self.vec_env.env.reset_done()
            ...
            if "base_actions" in res_dict:
                self.obs, rewards, self.dones, infos = self.env_step(
                    torch.cat([res_dict["base_actions"], res_dict["actions"]], dim=1)
                )
            else:
            ...
            if "total_rewards" in infos:
                self.current_rewards = torch.max(self.current_rewards, infos["total_rewards"][:, None])  # ! trick
            if "total_steps" in infos:
                self.current_lengths = torch.max(self.current_lengths, infos["total_steps"])

            if "reward_dict" in infos:
                reward_dict = infos["reward_dict"]
                for k, v in reward_dict.items():
                    if k in self.current_reward_dict:
                        self.current_reward_dict[k] += v.unsqueeze(1)
                    else:
                        self.current_reward_dict[k] = v.unsqueeze(1)
            ...
            for k, v in self.current_reward_dict.items():
                if k in self.game_reward_dict:
                    self.game_reward_dict[k].update(v[env_done_indices])
                else:
                    self.game_reward_dict[k] = torch_ext.AverageMeter(1, self.games_to_track).to(self.ppo_device)
                    self.game_reward_dict[k].update(v[env_done_indices])
                self.current_reward_dict[k] = self.current_reward_dict[k] * not_dones.unsqueeze(1)
...
class MyContinuousA2CBase(MyA2CBase):
    def __init__(self, base_name, params):
        ...
        self.use_soft_clamp = self.config.get("use_soft_clamp", True)
    ...
    def preprocess_actions(self, actions):
        if self.clip_actions:
            if self.use_soft_clamp:
                clamped_actions = self.soft_clamp(actions, torch.ones_like(actions) * -1.0, torch.ones_like(actions))
            else:
    ...
    def prepare_dataset(self, batch_dict):
        ...
        if self.normalize_value:  # ! TODO: should mask the error values
        ...
        if self.normalize_advantage:
            if self.is_rnn:
                if rnn_masks is not None and error_masks is not None:
                    rnn_masks = rnn_masks * error_masks
                if self.normalize_rms_advantage:
                ...
                if self.normalize_rms_advantage:
                    if error_masks is not None:
                        advantages = self.advantage_mean_std(advantages, mask=error_masks)
                    else:
        ...
        while True:
            ...
            if self.global_rank == 0:
                ...
                if self.game_fails.current_size > 0:
                    mean_fail_rate = self.game_fails.get_mean()
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if mean_success_rate is not None:
                        checkpoint_name += f"_sr_{mean_success_rate}"
                ...
                if epoch_num >= self.early_stop_epochs:
                    ...
                    if slope_early_stop:
                        ...
                        if mean_success_rate is not None:
                            saving_checkpoint_name += f"_sr_{mean_success_rate}"
                        if mean_fail_rate is not None:
                            saving_checkpoint_name += f"_fr_{mean_fail_rate}"

                        self.save(os.path.join(self.nn_dir, saving_checkpoint_name))
                        print("EARLY STOP!")
                        should_exit = True
