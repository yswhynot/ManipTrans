...
from lib.rl.base import MyContinuousA2CBase
...
class PPOAgent(MyContinuousA2CBase):
    ...
    def calc_gradients(self, input_dict):
        ...
        error_masks = input_dict.get("error_masks", None)
        ...
        if error_masks is not None and rnn_masks is not None:
            cur_mask = error_masks * rnn_masks
        elif error_masks is not None:
            cur_mask = error_masks
        ...
        with torch.no_grad():
            reduce_kl = cur_mask is None
            kl_dist = torch_ext.policy_kl(mu.detach(), sigma.detach(), old_mu_batch, old_sigma_batch, reduce_kl)
            if cur_mask is not None:
                kl_dist = (kl_dist * cur_mask).sum() / cur_mask.numel()  # / sum_mask
