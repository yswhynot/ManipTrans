...
from lib.utils.array import any_to_numpy, any_to_torch_tensor
...
class DataDict(collections.abc.MutableMapping):
    ...
    def __init__(self, _data_: collections.abc.Mapping = None, **kwargs):
        ...
        if _data_ is not None:
            ...
            assert _is_mapping(_data_) or is_iter, (
                f"data type {type(_data_)} is not supported. " f"Only Mapping and Iterable[(key, value)] are valid."
            )
            assert not kwargs, (
        ...
        if isinstance(key, str):
            if "." in key:
                ...
                if not hasattr(subdict, "__getitem__"):
                    raise KeyError(f"Parent is not a subscriptable object, " f'cannot access child key "{child_key}"')
    ...
    def to_torch_tensor(
        self,
        dtypes: Union[Dict[str, Union[str, torch.dtype]], str, torch.dtype, None] = None,
        copy: bool = False,
