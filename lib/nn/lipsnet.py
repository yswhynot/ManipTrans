...
def build_lipsnet(
    ...
    return LipsNet(
        f_sizes=[input_size] + units,
        f_hid_nonliear=nn.ELU,
        f_out_nonliear=nn.ELU,
        global_lips=False,
        k_init=1,
        k_sizes=[input_size] + [128, 32] + [1],
        k_hid_act=nn.Tanh,
...
if __name__ == "__main__":
    ...
    output_dim = 2
    net = LipsNet(
        f_sizes=[intput_dim, 64, 64, output_dim],
        f_hid_nonliear=nn.ReLU,
        f_out_nonliear=nn.Identity,
        global_lips=False,
        k_init=1,
    ...
    optimizer = torch.optim.Adam(
        [{"params": net.f_net.parameters(), "lr": 3e-5}, {"params": net.k_net.parameters(), "lr": 1e-5}]
    )
